...
- `$MEMORY_DIR` is `~/.claude/projects/{slug}/memory`, where `{slug}` is the absolute workspace path with `/` replaced by `-`. For example, if the workspace is at `/home/user/my-workspace`, the slug is `-home-user-my-workspace`, so `$MEMORY_DIR` is `~/.claude/projects/-home-user-my-workspace/memory`.
